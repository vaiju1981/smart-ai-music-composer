...
4. **Which bars it missed.** A fixed priority over the thirteen metrics. This is the
...
METRIC_TIERS: Final[tuple[str, ...]] = (
    # What a listener hears as "the tune": a line that moves mostly by step and
    # not only by step, has leaps in it to answer, answers them, and never jumps
    # further than an octave. The step bar and the leap bar are one vocabulary
    # read from both sides, so they rank together — and the leap bar above the
    # answering rule, because a leap that never happens cannot be answered.
    "step_ratio",
    "leap_ratio",
    "leap_recovery_ratio",
    "max_leap_semitones",
...
"""The thirteen metrics, most important first. The order the arbiter ranks a miss by.
...
def _breach(finding: QualityFinding) -> float:
    ...
    A fraction, because the thirteen metrics are not in the same units: a semitone
    ...
    since all thirteen bars are positive, so no value through `findings()` can
